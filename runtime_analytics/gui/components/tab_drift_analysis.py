...
def render(tab):
    with tab:
        ...
        st.dataframe(
            df_filtered[
                ["timestamp", "job_order", "job_sequence", "duration", "predicted_duration", "duration_seq_delta", "predicted_seq_delta"]
            ].tail(50)
        )

        st.markdown("#### RMSE by Job Order")
        # Step 1: Group by 'job_order' and calculate RMSE for each group
        rmse_values = df_filtered.groupby("job_order").apply(lambda g: mean_squared_error(g["duration"], g["predicted_duration"]))  # Calculate RMSE

...
def render(tab):
    with tab:
        ...
        if not model_path.exists():
            st.warning(
                "Prediction model not found. Please train the model first using the Admin tab.")
            return
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                if "job_logs_with_predictions" not in table_names["name"].values:
                    st.warning(
                        "Predicted results not found. Please run predictions from the Admin tab after training the model.")
                    return
            ...
            df_pred["timestamp"] = pd.to_datetime(
                df_pred["timestamp"], format="%Y-%m-%d %H:%M:%S,%f", errors="coerce")
        ...
        days = st.slider("Select number of days to include",
        ...
        if anomalies.empty:
            ...
            st.dataframe(
                anomalies[["run_date", "job_order", "job_sequence", "sequence_diff"]])
            st.download_button("Download Anomalies CSV", data=anomalies.to_csv(
                index=False), file_name="sequence_anomalies.csv")

        st.markdown("#### Prediction Drift Over Time")
        st.line_chart(df_filtered.set_index("timestamp")[
                      ["duration", "predicted_duration"]].sort_index())
        ...
        rmse_values = df_filtered.groupby("job_order").apply(
            lambda g: mean_squared_error(
                g["duration"], g["predicted_duration"])
        )  # Calculate RMSE

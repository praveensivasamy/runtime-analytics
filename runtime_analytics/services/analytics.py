...
def filter_jobs(df: pd.DataFrame, filters: Optional[dict[str, object]] = None, **kwargs) -> pd.DataFrame:
    if not filters:
        return df
...
def top_slow_jobs_grouped(df: pd.DataFrame, n: int = 10, **kwargs) -> pd.DataFrame:
    ...
    top_jobs = (
        df.groupby(["job_id", "timestamp"], as_index=False)["duration"]
        .max()
    ...
    return df_top[[
        "rank", "timestamp", "riskdate", "id", "type", "predicted_duration", "duration",
        "config_count", "job_order", "job_run_count", "job_sequence", "job_count",
        "day", "month", "week", "year", "week_number"
...
def top_anomaly_scores(df: pd.DataFrame, n: int = 10, **kwargs) -> pd.DataFrame:
    if "anomaly_score" not in df.columns:
        return pd.DataFrame(columns=[
            "riskdate", "id", "type", "anomaly_score",
            "run_timestamp", "config_count", "run_id"
        ])

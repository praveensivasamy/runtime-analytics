...
def aggregate_by_field(df: pd.DataFrame, group_by: str, agg_field: str, operations: list[str]) -> pd.DataFrame:
...
def filter_jobs(df: pd.DataFrame, filters: Optional[dict[str, object]] = None) -> pd.DataFrame:
    if not filters:
        return df
...
def top_slow_jobs_grouped(
    df: pd.DataFrame, n: int = 10, date_filter: Optional[str] = None, start_date: Optional[str] = None, end_date: Optional[str] = None
) -> pd.DataFrame:
    df = df.copy()
    ...
    # Group by unique job_id + timestamp to get N the slowest exact job executions
    top_jobs = df.groupby(["job_id", "timestamp"], as_index=False)["duration"].max().sort_values(by="duration", ascending=False).head(n)

    # Assign contextual rank within this filtered set
    top_jobs["rank"] = range(1, len(top_jobs) + 1)

    # Join back with filtered df on job_id + timestamp
    df_top = df.merge(top_jobs[["job_id", "timestamp", "rank"]], on=["job_id", "timestamp"], how="inner")

    # Add time metadata fields
...
def prediction_accuracy_per_job_type(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .agg(
            total_runs=("id", "count"),
            avg_absolute_error=("predicted_duration", lambda x: (x - df.loc[x.index, "duration"]).abs().mean()),
            avg_relative_error=(
                "predicted_duration",
                lambda x: ((x - df.loc[x.index, "duration"]).abs() / (df.loc[x.index, "duration"] + 1e-5)).mean(),
            ),
        )
        .reset_index()
    )
